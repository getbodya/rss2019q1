export default class Frame{
  ...
  setState() {
    const {frames} = state;
      ...
      const newFrame = Object.assign({},framesList[0]);
      framesList.push(newFrame);
      state.frames.framesList = framesList;
  }
  addFrame(data){
    ...
    const deleteFrameBtn = Frame.makeDeleteFrameBtn()
    const copyFrameBtn = Frame.makeCopyFrameBtn(frame);
    if(data){
      const ctx = frame.getContext('2d');
      ctx.putImageData(data,0,0)
    }
